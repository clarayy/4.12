...
#感染过程
    ...
    for i in range(Roundtime):
        for nbr, datadict in G.adj.items():#遍历G的所有节点，nbr节点名称，datadict与节点相连的边
            if int(nbr) in S:
                ...
                for key in datadict:
                    #if int(key) in I and int(key) not in statechange1:   ##I变成R以后在同一时间步 不能感染其他节点  不一定需要
                    if int(key) in I:
                        node_adj=node_adj+1
    ...
    filename_center = perfix + '_jordancenter.txt'   #默认为随机选择jordan center
    filename_center1 = perfix + '_jordancenter1.txt'  #记录的是jordan center第一个元素
    ...
    if not nx.is_empty(new_G):
        ...
        # #用最大连通子图输入GNN
        # GNN_G = nx.Graph()
        # GNN_G.add_nodes_from(i for i in range(N))
        # GNN_G.add_edges_from(Gc.edges())
        # adj_matrix = nx.adjacency_matrix(GNN_G).todense()
        #无偏中介中心性
...
def data_A(datadir,bmname):
    ...
    with open(filename_readme,'a') as f:
        ...
        f.write('RecoverRate='+str(RecoverRate)+"\n")
...
def main():
    ...
    with open(filename_readme,'w') as f:
        #f.write('propram pro GNN and data'+"\n")
