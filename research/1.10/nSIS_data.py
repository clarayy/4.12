#propagation5_lei_jian_SIS_data.py  中省略了三种方法的对比，在此程序中恢复
import numpy as np
import pandas as pd
...
#感染过程
    ...
    if not nx.is_empty(new_G):
        ...
        every_jc = []
        for c in nx.connected_components(new_G_small): #所有联通子图
            #nodeSet = new_G_small.subgraph(c).nodes()
            subgraph = new_G_small.subgraph(c)
    ...
    #print(adj_matrix.shape)
        ...
        with open(filename_center1,'a') as centerf1:
            ...
            for i in every_jc:                                    #记录下所有的jordancenter，包括所有子图的所有jc
...
def data_A(datadir,bmname):
   ...
   # nodelist = [10, 11, 12, 24, 25, 26, 27, 28, 43, 54, 55, 59, 62, 65, 66, 81, 83, 84, 96, 105, 107, 172, 174, 180, 181, 182, 183, 184, 186, 188, 205, 207, 232, 233, 237, 239, 240, 242, 243, 244, 246, 263, 274, 287, 291, 298, 324, 353, 374, 382, 390, 393, 409, 410, 413, 423, 425, 428, 430, 435, 455, 472, 474, 478, 479, 485]
    ...
    class0= [0, 1, 3, 5, 6, 202, 203, 229, 230, 231, 326, 327, 328, 329, 330, 345, 346, 347, 348, 368, 399, 400, 402, 403, 404, 415, 450, 451, 471]
    class1= [2, 4, 10, 11, 12, 24, 25, 26, 27, 28, 43, 54, 65, 66, 92, 93, 94, 95, 96, 98, 105, 172, 174, 179, 180, 181, 182, 183, 184, 186, 188, 205, 207, 208, 239, 240, 243, 244, 245, 246, 249, 257, 260, 261, 262, 274, 284, 286, 287, 289, 290, 291, 295, 298, 324, 361, 363, 382, 390, 393, 398, 409, 410, 411, 413, 423, 424, 425, 426, 428, 429, 430, 435, 455, 472, 474, 478, 479]
    class2= [226, 372, 487]
    class3= [13, 14, 15, 16, 18, 19, 20, 21, 22, 23, 85, 191, 192, 193, 194, 383, 394, 449]
    class4= [17, 77, 79, 110, 111, 161, 162, 163, 164, 166, 167, 168, 169, 171, 173, 247, 252, 297, 369, 370, 391, 416, 417, 418, 419, 458, 459, 473, 486, 490]
    class5= [187, 320, 321, 414, 491]
    class6= [31, 47, 51, 52, 53, 55, 56, 57, 58, 59, 60, 61, 62, 63, 64, 67, 68, 69, 70, 71, 72, 73, 80, 82, 83, 97, 104, 107, 108, 109, 134, 141, 165, 170, 175, 176, 177, 178, 209, 223, 225, 227, 232, 233, 234, 235, 236, 241, 258, 259, 263, 265, 268, 271, 272, 273, 275, 276, 300, 301, 335, 353, 364, 374, 384, 385, 386, 461, 470, 475, 485, 495]
    class7= [211, 212, 213, 214, 215, 444]
    class8= [38, 112, 113, 116, 118, 119, 120, 121, 122, 124, 126, 127, 128, 129, 130, 131, 132, 135, 136, 137, 139, 145, 146, 147, 148, 149, 150, 151, 152, 153, 154, 155, 156, 157, 158, 159, 160, 189, 216, 217, 218, 254, 255, 277, 278, 288, 314, 317, 318, 319, 343, 344, 373, 397, 405, 406, 407, 437, 462, 463, 464, 465, 480, 483, 488, 489, 492, 494, 496, 497, 498, 499]
    class9= [39, 40, 41, 42, 44, 45, 114, 115, 219, 220, 221, 222, 322]
    class10= [33, 34, 35, 36, 37, 46, 48, 49, 50, 75, 76, 78, 308, 309, 310, 311, 312, 313, 336, 337, 338, 339, 378, 379, 387, 388, 389, 396, 412, 432, 469, 481]
    class11= [7, 8, 9, 29, 30, 32, 81, 84, 86, 87, 88, 89, 90, 91, 106, 117, 123, 125, 133, 138, 140, 142, 143, 144, 185, 190, 195, 196, 197, 198, 199, 200, 201, 206, 210, 224, 228, 237, 238, 242, 250, 251, 253, 266, 279, 280, 285, 296, 307, 316, 323, 331, 332, 333, 334, 340, 341, 342, 351, 352, 354, 355, 356, 357, 358, 359, 360, 362, 365, 366, 375, 381, 392, 401, 408, 422, 427, 431, 433, 434, 436, 438, 439, 440, 441, 442, 443, 445, 446, 447, 448, 456, 457, 460, 466, 467, 468, 477, 482, 484]
    class12= [74, 99, 100, 101, 102, 103, 204, 248, 256, 264, 267, 269, 270, 281, 282, 283, 292, 293, 294, 299, 302, 303, 304, 305, 306, 315, 325, 349, 350, 367, 371, 376, 377, 380, 395, 420, 421, 452, 453, 454, 476, 493]
    #nodelist = class0
...
def main():
    ...
    bmname = 'food500_nSIS_z0.1_m2_testjc'
