...
plt.show()
G_NN = nx.Graph()
G_NN.add_nodes_from(i for i in range(N))
G_NN.add_nodes_from(new_G.nodes())    #不增加单独节点看实验效果如何，max_nodes=100时，max graph size 是否为100
G_NN.add_edges_from(new_G.edges())
A = nx.to_numpy_matrix(G_NN) 
print(A)