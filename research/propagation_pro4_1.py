...
#感染过程
    ...
    if not nx.is_empty(new_G):
        ...
        center_frozen_graph = nx.freeze(sub_newG)#G被冷冻为frozen_graph，不会改变
        ...
        #无偏中介中心性
        bet_cen = nx.betweenness_centrality(sub_newG)#节点的中介中心性
        deg = sub_newG.degree()#节点的度
        ub={}
        ...
        #distance centrality
        p = dict(nx.shortest_path_length(sub_newG))
        d={}
        for k,v in p.items():
            s=0
            for i in v.values():
        ...
        #dynamic ages
        frozen_graph = nx.freeze(sub_newG)#G被冷冻为frozen_graph，不会改变
        ...
        with open(filename_dynage,'a') as dynagef:
            ...
            dynagef.write('\n')
        # nx.draw(sub_newG,with_labels = True)
        # plt.show()
...
def main():
    ...
    with open(filename_readme,'w') as f:
        f.write("bmname:"+bmname+'\n')
        f.write("part observation")
        f.write('N='+str(N)+"\n")
