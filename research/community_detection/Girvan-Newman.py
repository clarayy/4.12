...
class GN:
    ...
    def write_npy(self,f):
        dict_par = {}
        for i in range(len(self.partition)):#i=0~9
            for value in self.partition[i]:
                dict_par[value] = i
            i = i+1
        #print(dict_par)
        np.save('/home/iot/zcy/usb/copy/research/graph_python/research/community_detection/'+f+"_gn2.npy",dict_par)   
...
if __name__ == '__main__':
    ...
    N = 1000
    fname = 'BA1000'
    B = np.load("/home/iot/zcy/usb/copy/research/graph_python/"+fname+".npy")#读取固定的图
    G = nx.Graph(B)
    ...
    # #G2=nx.read_gml('karate.gml',label='id') #Dividing communities until each node is a community
    # G2 = nx.Graph(B)
