    ...
    for i in range(Roundtime):
        for nbr, datadict in G.adj.items():#遍历G的所有节点，nbr节点名称，datadict与节点相连的边
            if int(nbr) in I:              #I节点的感染邻接点数
                ...
                if random.random() <= RecoverRate and int(nbr)!= start_node: #同一时间轮时，先感染后恢复成S，独立吗？？
                    ItoR.append(int(nbr))
