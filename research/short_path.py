...
#pycharm中的文件名
bmname = 'dolphins_nSI_z0.1_m30'
path = os.path.join('/home/iot/zcy/usb/copy/rexying_diffpool/diffpool-master/labels',bmname)
file_labels = path + '_labels.txt'
file_preds = path + '_preds.txt'
file_val_jordancenter = path + '_val_jordancenter.txt'
...
print('平均距离中心距离：{:.4}'.format(average_distance_discen))
count = 0
for i in range(len(distance_preds)):
...
dis_preds_frequence = []
for i in cnt:
    dis_preds_frequence.append(i/(len(preds)))
#fig, ax = plt.subplots()
plt.figure()
...
#path_fig = os.path.join('/home/zhang/Documents/research/graph_python/research/experiments_graph',bmname) + '_results.png'
plt.tight_layout()
